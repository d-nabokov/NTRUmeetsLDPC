...
/* ----- arithmetic mod 3 */
// extern int sec_index[u_column_size];
// 声明外部变量
extern int u_column_size;
extern int *sec_index;


extern int c_value_for_attack_list_1[8][3];
extern int c_value_for_attack_list_2[8][3];
extern int param_list1_index;
extern int param_list2_index;
extern int param_which_list;
extern int Beta_which;
...
// 定义外部变量
int pq_counter;
int inner_test_counter;
int noninvertible_count;
...
static uint16 urandom16(void)
{
    unsigned char c[2];
    uint16 out[2];

    randombytes(c, 2);    // 生成2个字节的随机数
    out[0] = (uint16)c[0];        // 低8位
    out[1] = ((uint16)c[1]) << 8; // 高8位
    return out[0] + out[1];       // 将两部分相加，得到16位的随机数
}
...
static void Encrypt(Fq *c,const small *r,const Fq *h)
{
  ...
  {
      FILE *d1_d2_data_file = fopen("d1_d2_data.txt", "a");

      if (d1_d2_data_file == NULL) 
      {
        perror("Failed to open file: d1_d2_data\n");
        if (d1_d2_data_file != NULL) fclose(d1_d2_data_file);
        return;
      }
      ...
      for(int we1 = 0; we1 < m; we1++)
      {
        ...
        while(found == 0)
        {
          ...
          if(index >= (p/2) && index < p )
            found = 1;

        }
        //printf("Index of f1: %d\n", index);
        x_f_array[index] = 1;
        non_zero_f_coeff = index;
         index_array_f[we1] = index;  // 记录index
      }
      ...
      for(int sd = 0; sd < p; sd++)
      {
        ...
        hr_3_base[sd] = F3_freeze(hr[sd]);
      }
      printf("c_value_1 is %d c_value_2 is %d\n", c_value_1, c_value_2);
      ...
      // if (no_collisions_pos == 1 || no_collisions_neg == 1)
      // { 
        for(int sd = 0; sd < p; sd++)
        {
          global_f_3[sd]=3*global_f[sd];
        }
       

        Rq_mult_small(noise1,global_f_3,hr_3_base);

        fprintf(d1_d2_data_file, "pq_counter: %d", pq_counter);
        fprintf(d1_d2_data_file, "\nindex_array of x_f_array_copy : ");
        for (int i = 0; i < m; i++) {
            fprintf(d1_d2_data_file, "%d,", index_array_f[i]);
        }
    
        fprintf(d1_d2_data_file, "\nx_f_array_copy: ");
        for (int i = 0; i < p; i++)
        {
            fprintf(d1_d2_data_file, "%d,", x_f_array_copy[i]);
        }

        fprintf(d1_d2_data_file, "\nindex_array of x_g_array_copy : ");
        for (int i = 0; i < n; i++) {
            fprintf(d1_d2_data_file, "%d,", index_array[i]);
        }
        fprintf(d1_d2_data_file, "\nx_g_array_copy: ");
        for (int i = 0; i < p; i++)
        {
            fprintf(d1_d2_data_file, "%d,", x_g_array_copy[i]);
        }
        fprintf(d1_d2_data_file, "\nglobal_g: ");
        for (int i = 0; i < p; i++)
        {
            fprintf(d1_d2_data_file, "%d,", global_g[i]);
        }
  ...
  if(intended_function == 1)
  {
    ...
    for(int j=0; j< u_column_size; j++)
    {
      x_f_attack_array[sec_index[j]] += 1;
      //printf("sec_index[%d] = %d\n", j, sec_index[j]);  // 打印 sec_index[j] 的值
    }
    // printf("In kem_mod.c sec_index are %d\n", sec_index[0]);

    // // printf("\n");
    // printf("Printing x_f_attack_array u_column_size is:%d \n", u_column_size);
    // for(int i = 0; i < p; i++)
    // {
    //   printf("%d, ", x_f_attack_array[i]);
    // }
    ...
    if (param_which_list == 0) 
    {
        c_value_for_now_1 = c_value_for_attack_list_1[param_list1_index][0];
        c_value_for_now_2 = c_value_for_attack_list_1[param_list1_index][1];
        c_value_for_now_3 = c_value_for_attack_list_1[param_list1_index][2];
    } 
    else 
    {
        c_value_for_now_1 = c_value_for_attack_list_2[param_list2_index][0];
        c_value_for_now_2 = c_value_for_attack_list_2[param_list2_index][1];
        c_value_for_now_3 = c_value_for_attack_list_2[param_list2_index][2];
    }
    ...
    for(int sd = 0; sd < p; sd++)
    {
      temp_value = (x_f_array[sd])*c_value_for_now_1 + (x_g_int[sd])*c_value_for_now_2 + (x_f_attack_array[sd]*mul_const*c_value_for_now_3);
      //printf("%d,",temp_value); 
      hr[sd] = Fq_freeze(temp_value);
      hr_3_att[sd] = F3_freeze(hr[sd]);
    }
    ...
    Rq_mult_small(cf_noround, global_c_in_encrypt, global_f);
    
    Rq_mult3(cf3_noround, cf_noround);
    R3_fromRq(e_noround, cf3_noround);
    ...
    Rq_mult_small(noise2,global_f_3,hr_3_att);
