...
#define MAX_OUTPUT_SIZE 1277  
#define OUTPUT_WEIGHT_FILENAME "/home/fei/C/SCA_Assisted_CCA_on_NTRU/NTRU_Prime/PC_Oracle_based_SCA/Attack_Simulations/sntrup761/SCA-LDPC/simulate-with-python/fprime_weight.txt" // 输出文件的绝对路径
...
double average_match_table[9][1] = {
    {0.03635135276205638}, // -4
    {0.6577072454679685}, // -3
    {9.657621149950542},  // -2
    {47.313982601285154},  // -1
    {84.66867530106856},  // 0
    {47.313982601285154},  // 1
    {9.657621149950542},  // 2
    {0.6577072454679685},  // 3
...
void update_match_table_2(double counts[1][2], int new_array_size) {
    for (int col = 0; col < 1; col++) { 
        ...
        for (int y_index = 0; y_index < 3; y_index++) {
            ...
            if (y_index == 0) {
                // 第一列 (y_index == 0): expected_zeros = column_count * (1 - 0)，expected_minus_ones = column_count * 0
                expected_zeros = new_array_size * (1 - 0);
                expected_minus_ones = new_array_size * 0;
            } else if (y_index == 1) {
                // 第二列 (y_index == 1): expected_zeros = column_count * (1 - column_factor)，expected_minus_ones = column_count * column_factor
                expected_zeros = new_array_size * (1 - 0.17951202373168398);
                expected_minus_ones = new_array_size * 0.17951202373168398;
            } else if (y_index == 2) {
                // 第三列 (y_index == 2): expected_zeros = column_count * 0，expected_minus_ones = column_count * 1
...
void calculate_beta_probabilities(long double *Pr, int beta_coeff_count) {
    ...
    if (beta_coeff_count == 1) {
        ...
        Pr[2] = 60516.0L / 1630729.0L;
        Pr[3] = 386220.0L / 1630729.0L;
        Pr[4] = 737257.0L / 1630729.0L;
        Pr[5] = 386220.0L / 1630729.0L;
        Pr[6] = 60516.0L / 1630729.0L;
    } else if (beta_coeff_count == 3) {
        Pr[1] = 14886936.0L / 2082440933.0L;
        Pr[2] = 142515180.0L / 2082440933.0L;
        Pr[3] = 499434858.0L / 2082440933.0L;
        Pr[4] = 768766985.0L / 2082440933.0L;
        Pr[5] = 499434858.0L / 2082440933.0L;
        Pr[6] = 142515180.0L / 2082440933.0L;
        Pr[7] = 14886936.0L / 2082440933.0L;
    } else if (beta_coeff_count == 4) {
        Pr[0] = 3662186256.0L / 2659277071441.0L;
        Pr[1] = 46744979040.0L / 2659277071441.0L;
        Pr[2] = 238397577624.0L / 2659277071441.0L;
        Pr[3] = 616231776120.0L / 2659277071441.0L;
        Pr[4] = 849204033361.0L / 2659277071441.0L;
        Pr[5] = 616231776120.0L / 2659277071441.0L;
        Pr[6] = 238397577624.0L / 2659277071441.0L;
        Pr[7] = 46744979040.0L / 2659277071441.0L;
        Pr[8] = 3662186256.0L / 2659277071441.0L;
    }
}
    ...
    while (*success_trial == 0) 
    {
        ...
        *collision_count = 0; 
        ...
        {           
            ...
            randombytes(random_value_bytes, sizeof(random_value_bytes));  // 获取 4 字节的随机数
            ...
            if (random_value >= noise_probability) {
                ...
                randombytes(noise_type_bytes, sizeof(noise_type_bytes));  // 获取 1 字节的随机数
...
long double **init_oracle_accuracy() {
    ...
    oracle_accuracy[0][0] = 0.042000L;  // False positive probability
    oracle_accuracy[0][1] = 0.042000L;  // False negative probability
...
int main()
{
    ...
    FILE *d1_d2_data_file_txt_txt = fopen("d1_d2_data.txt", "w");
    if (d1_d2_data_file_txt_txt == NULL) {
        perror("Failed to open file");
        return 1;
    }
    fclose(d1_d2_data_file_txt_txt);
            ...
            if(max_distance >= 35)
            {
              float weight_score = 1.0 *abs(q12 - value1) + 1.0 * max_distance;
                ...
                if(touch_np == 0)
                {
                    ...
                    if (dist_value == 1 && weight_score >= 90 && abs(q12 - value1) >= max_distance)
                    {
                    ...
                    else if (dist_value == 2 && weight_score >= 180 && abs(q12 - value1) >= max_distance)
                    {
                        c_value_for_attack_1_1[dist_value - 1] = hg;
                        c_value_for_attack_1_2[dist_value - 1] = hg1;
                        c_value_for_attack_1_3[dist_value - 1] = hg2;
                    ...
                    else if (dist_value == 3 && weight_score >= 305 && abs(q12 - value1) >= max_distance)
                    {
                        c_value_for_attack_1_1[dist_value - 1] = hg;
                        c_value_for_attack_1_2[dist_value - 1] = hg1;
                        c_value_for_attack_1_3[dist_value - 1] = hg2;
    ...
    //    {
                ...
                while (cond_prob_loop < 1)
                {
                        ...
                        {  
                            ...
                            {                                   
                                ...
                                for (int col = 0; col < MAX_ROWS; col++) 
                                {
                                    ...
                                    if (oracle_response_2_col_sizes[col] > 0) 
                                    {
                                        ...
                                        if ((current_tree_state == 1 && val_0 == 0) || (current_tree_state == 2 && val_0 == -1)) {
                                            ...
                                            updated_index_array_2 = (int *)realloc(updated_index_array_2, (updated_index_array_size_2 + 1) * sizeof(int));  // 扩展 updated_index_array_2
                                            ...
                                            updated_sec_index_second_part[updated_array_size_2][0] = sec_index_second_part[col][0];  // 存储二维数组的第一列
                                            updated_sec_index_second_part[updated_array_size_2][1] = sec_index_second_part[col][1];  // 存储二维数组的第二列
                                            ...
                                            index_array_2 = (int *)realloc(index_array_2, (index_array_size_2 + 1) * sizeof(int));  // 扩展 index_array_2
                                            ...
                                            new_sec_index_second_part[new_array_size_2][0] = sec_index_second_part[col][0];  // 存储二维数组的第一列
                                            new_sec_index_second_part[new_array_size_2][1] = sec_index_second_part[col][1];  // 存储二维数组的第二列
                                ...
                                for (int i = 0; i < updated_array_size_2; i++) {
                                    sec_index_second_part[i][0] = updated_sec_index_second_part[i][0];  // 复制第 0 列
                                    sec_index_second_part[i][1] = updated_sec_index_second_part[i][1];  // 复制第 1 列
                                }
                                    ...
                                    for (int col = start_idx; col < oracle_response_2_capacity; col++) 
                                    {
                                        ...
                                        if (oracle_response_2_col_sizes[col] > 0) 
                                        {
                                            ...
                                            if ((current_tree_state == 1 && val_0 == 0) || (current_tree_state == 2 && val_0 == -1)) {
                                                ...
                                                filtered_check2_array[filtered_array_second_length] = 1; // 标记为符合筛选条件
                    ...
                    // Generate the first tree from array
                    ...
                    // Output encoding for coding_tree_1
                    ...
                    // Loop through and sample coefficients for coding_tree_1
                    for (int s = -sum_weight; s <= sum_weight; s++) 
                    {
                        ...
                        printf("%d \t| (", s);
                        fprintf(actual_match_table_file, "%d \t| (", s);  // 同时写入文件

                        for (int i = 0; i < size; i++) {
                            printf("%d", out[i]);
                            fprintf(actual_match_table_file, "%d", out[i]);  // 同时写入文件

                            if (i < size - 1) {
                                printf(", ");
                                fprintf(actual_match_table_file, ", ");  // 同时写入文件
                            }
                        }
                        printf(")\n");
                        fprintf(actual_match_table_file, ")\n");  // 同时写入文件
                    }
                    printf("Chosen encoding1_check\n");
                    fprintf(actual_match_table_file, "tree_1_check:\n");  // 同时输出到文件
                    ...
                    for (int idx1 = 0; idx1 < u_1_u_2; idx1++) 
                    {
                        ...
                        printf("beta_u: %d, oracle output: (", beta_u);
                        for (int i = 0; i < size; i++) {
                            printf("%d", out[i]);
                            if (i < size - 1) {
                                printf(", ");
                ...
                fprintf(overall_oracle_calls_count_file, "overall_oracle_countz is %d\n", overall_oracle_count);
