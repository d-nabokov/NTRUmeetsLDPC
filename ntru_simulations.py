...
def ldpc_decode(idxs, distrs, iterations, prior_secret_variables, sum_weight):
    ...
    s_decoded = decoder.decode_hard(secret_variables, check_variables)
    return s_decoded
...
# sum_weight = 2
# num_checks = NTRU_N
# checks = generate_regular_ldpc_as_tanner(NTRU_N, num_checks, sum_weight)
# coding_tree = tree_from_array([(True, 1), (False, -1), (True, 2), None, (False, -2)])
# attack_params.append((checks, coding_tree, sum_weight))

sum_weight = 6
num_checks = NTRU_N + 150
checks = generate_regular_ldpc_as_tanner(NTRU_N, num_checks, sum_weight)
...
pr_oracle = SimpleOracle(0.96)  # Bernoulli noise
...
oracle_calls_arr = []
key_num = 0
for key_idx in tqdm(range(TEST_KEYS), desc="Progress for processing secret keys"):
    key = keys[key_idx]
    collision_info = collisions[key_idx]
    f = to_centered(key)
    ...
    for u_list, coding_tree, sum_weight in attack_params:
        for u in u_list:
            ...
            ### NOTE: in paper's version we submit `distr` into LDPC instead of channel_pmf which is incorrect
            ### and leads to a worse result
            ...
            if sum_weight == max_sum_weight:
                cond_distr.append(channel_pmf)
            else:
                cond_distr.append(extend_distribution(channel_pmf, max_sum_weight))
            # if y == (1, 1) or y == (0, 1, 1):
            #     determined_set = determined_set.union(set(alpha))

    s_decoded = ldpc_decode(alpha_idxs, cond_distr, 10000, s_prior, max_sum_weight)
    fprime = s_decoded[:n]
